...
> This RMarkdown represents the Analytics Zone and consists of the analysis that has been done in the Video Metadata files.
> **Obs**: All the analysis performed throughout this R markdown is better explained in the written document. There it is detailed the motivations and reasoning for each process and section.
...
# Extra: Save the files in a permanent data in order to be easily readable
...
save(json_list,df_language_fulltitle,df_language_fulltitle_textcat, sorted_json_list3,df_json_metadata, df_json_metadata_enrich, res.pcamix,res.pca,videos_words,dataframeLikes,file="Permanentdata")
```

```{r eval = TRUE, echo = TRUE, warning = FALSE}
# Load the saved data. Warning: the setwd() must be pointing to the folder where the file is placed.
load(paste0(root_path,"Permanentdata"))
```

```{r eval = FALSE}
# This is not executed in the knitr part because it was previously saved in the permanent data. This process is slow, takes approximately 1h30min to create the json_list object, even using parallel library
...
#Parallel not working in RStudio https://github.com/rstudio/rstudio/issues/6692
...
## Testing the results using a few random samples (visual checking against the original data)
```{r warning = FALSE}
# Testing the results
...
# Attempt 1: not good therefore not used
'%notin%' <- Negate('%in%')
...
transcript_video_ids <- data.frame()
...
# Defining the global score
# First Technique: PCA

## Attempt 1
> This attempt was not finally successful because the way the data was organized and the formula was executed, was not finally correct because the indicators were not taken into account, only the variables separately. However we left the attempt here.
...
## Attempt 2
> This attempt was the one we considered successful, finally the one used for the project to define the global score

```{r warning = FALSE}
pca_data <- read.xlsx(file=paste0(root_path,"conjoint_data.xlsx"),2)
head(pca_data)

pca_data <- pca_data %>% dplyr::rename(video_id = VideoId)

#Converting ScoreVideo and ScoreChannel to numeric. Both data were character in excel
pca_data$ScoreVideo <- as.numeric(pca_data$ScoreVideo)
pca_data$ScoreChannel <- as.numeric(pca_data$ScoreChannel)
pca_data$SentimentPositiveVideo <- as.numeric(pca_data$SentimentPositiveVideo)

#2 steps executed in this pipeline: Calculate the LikeDislikeScore and SentimentScore and normalize the data to a maximum of 100
pca_data_enrich <- pca_data  %>% mutate(LikeDislikeScore=(LikeCountVideo-DislikeCountVideo)/ViewCount) %>% mutate(SentimentScore = (SentimentPositiveVideo - SentimentNegativeVideo)/ViewCount) %>% BBmisc::normalize(method = "range",range = c(0,100))
summary(pca_data_enrich)

#Execution of PCA function
# Scale = FALSE
res.pca.attempt2.scale.false <- PCA(pca_data_enrich,quanti.sup = c(1:2,5:9),scale.unit = FALSE)
#Print the first principal component
res.pca.attempt2.scale.false$svd$V[,1]

# Scale = TRUE - Disregarded
#res.pca.attempt2.scale.false <- PCA(pca_data_enrich,quanti.sup = c(1:2,5:9),scale.unit = FALSE)
#Print the first principal component
#res.pca.attempt2.scale.false$svd$V[,1]
```
...
# Second Technique: Conjoint Analysis
```{r warning = FALSE}
conjoint_data <- read.xlsx(file=paste0(root_path,"conjoint_data.xlsx"),1)
head(conjoint_data)
#summary(conjoint_data)

regression_multiple <- lm(~ScoreVideo+ScoreChannel+LikeDislikeScore+SentimentScore,data = conjoint_data)
