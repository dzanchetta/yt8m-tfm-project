...
#Since the like and dislike information is missing in the data, we merge with the specific dataset containing like and dislike information by video_id
pca_data_enrich <- merge(pca_data,dataframeLikes,by="video_id")

#Converting ScoreVideo and ScoreChannel to numeric. Both data were character in excel
pca_data_enrich$ScoreVideo <- as.numeric(pca_data_enrich$ScoreVideo)
pca_data_enrich$ScoreChannel <- as.numeric(pca_data_enrich$ScoreChannel)
...
pca_data_enrich <- pca_data_enrich  %>% mutate(LikeDislikeScore=(likeCount-dislikeCount)/viewCount) %>% mutate(SentimentScore = (SentimentPositiveVideo - SentimentNegativeVideo)/viewCount) %>% BBmisc::normalize(method = "range",range = c(0,100))
summary(pca_data_enrich)

#Execution of PCA function
res.pca.attempt2 <- PCA(pca_data_enrich,quali.sup =c(1:2,13:14),quanti.sup = c(5:12),scale.unit = FALSE)
#Print the first principal component
res.pca.attempt2$svd$V[,1]

#Screeplot
plot(res.pca.attempt2$eig[,1],type="l",main="Screeplot")
```

# Conjoint Analysis Attempt 1
```{r}
conn <- dbConnect(SQLite(),'/Users/daniel/Downloads/database (2).sqlite')
videos_words <- dbReadTable(conn,'videos_words') # 4590467 entries

sum_video_words <- ddply(videos_words,.(VideoId),summarize,sumTdIdf = sum(TFIDF)) # 53986 entries

sum_video_words_enrich_sample <- df_json_metadata_enrich_sample %>% mutate(sumTfIdf=ifelse(id %in% sum_video_words$VideoId,paste(sum_video_words$sumTdIdf),NA_character_))
sum_video_words_enrich_sample <- sum_video_words_enrich_sample[complete.cases(sum_video_words_enrich_sample),] # 859 entries after NA removal

conjoint <- lm(sumTfIdf~average_rating + dislike_count + duration + like_count + view_count + positive_comment + neutral_comment + negative_comment,data=sum_video_words_enrich_sample)
summary(conjoint)
```

# Conjoint Analysis Attempt 2
```{r}
